...
Divide and conquer approach is O(n logn).
...
module.exports = {
  ...
  maxSubArray : function(A){
    var max = this.maxSubArrayRecursive(A, 0, A.length-1);
    return max;
  },

  maxSubArrayRecursive : function(A, low, high) {
    ...
    var maxLeft = this.maxSubArrayRecursive(A, low, mid);
    var maxRight = this.maxSubArrayRecursive(A, mid+1, high);
    var c = this.maxSubArrayHelper(A, low, mid, high);

    return Math.max(maxLeft, Math.max(maxRight, c));

  },

  // recursive method uses divide and conquer approach
  maxSubArrayHelper : function(A, low, mid, high) {
    var sum = 0;
    var leftMax = Number.NEGATIVE_INFINITY;
    // elements to the left of mid
    for (var i = mid; i >= low; i--) {
      sum += A[i];
      leftMax = Math.max(leftMax, sum);
    }

    // elements to the right of mid
    sum = 0;
  ...
  // Brute force approace is O(n^2)
  bruteForce : function(A) {
    var max = Number.MIN_VALUE;

    // brute force .. find all sub arrays..
...
console.log(module.exports.maxSubArray(A));  // expect 6

A = [ -163, -20 ];
console.log(module.exports.maxSubArray(A));  // expect -20
...
console.log(module.exports.maxSubArray(A));  // expect 217